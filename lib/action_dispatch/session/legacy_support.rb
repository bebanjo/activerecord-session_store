    ...
    module LegacySupport
      ...
      def destroy_session(env, session_id, options)
        ...
        if sid = current_session_id(request.env)
          session, _sid = get_session_model(request, sid)
          session.destroy
      ...
      def get_session_model(request, sid)
        if request.env[self.class::ENV_SESSION_OPTIONS_KEY][:id].nil?
          ...
          session = request.env[self.class::SESSION_RECORD_KEY] || find_session(sid)
        end
        request.env[self.class::SESSION_RECORD_KEY] = session
        [session, sid]
      end

      def find_session(sid)
        get_session_with_fallback(sid) || build_session(sid, data: {})
      end
    end
  end
end
